...
import JobNav from './JobsNav'
import AddJob from "../jobManagementComponents/AddJob";
import CreateCv from "../cvBuilderComponents/CreateCv";

export default function ResponsiveJM({ search = false, applied = false, saved = false, myjobs = false, mycv = false, isAnyChange, setIsAnyChange }) {
    const [addJob, setAddJob] = React.useState(false)
    const [createCv, setcreateCv] = React.useState(false)
    ...
    const list = (anchor) => (
        ...
        >
            ...
            {/* yahase */}
            <JobNav rjm isAnyChange={isAnyChange} setIsAnyChange={setIsAnyChange} />
        </Box>
    );
